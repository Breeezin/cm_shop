<?php
	if( !IsSet( $_POST['report'] ) && !IsSet( $_GET['report'] ) )
		{
		header( "Location:se_report_list.php" );
		exit;
		}
	...
	if( IsSet( $action ) && ( $action == "update" ) )
		{
		$query = "UPDATE reports set re_label = \"".$re_label."\" "
    ...
    if( IsSet( $action ) && ( $action == "remove" ) )
		...
		// now what?
		echo "<td><a href=\"se_report_list.php\">Back</a></td>";
		echo "</html>";
		exit;
        }
...
//	echo $query;
	...
	$query = "SELECT * FROM reports where re_report =".$report;
	...
	echo "<form ACTION=\"se_report_edit.php\"METHOD=POST NAME=\"ReportEdit\">";
	...
	$query = "SELECT max(re_report) as re_report FROM reports where re_report < ".$report;
	...
	$query = "SELECT min(re_report) as re_report FROM reports where re_report > ".$report;
	if( ($result = mysql_query( $query )) == false)
		{
		echo "Error selecting from reports -- " . mysql_error();
		exit;
	...
	echo "<form ACTION=\"se_report_edit.php\" METHOD=POST>";
	echo "<input type=\"hidden\" value=\"".$report."\" name=\"report\">";
	...
	if( IsSet($previous) && strlen( $previous ) > 0 )
		echo "<td><a href=\"se_report_edit.php?report=".$previous."\">Previous</a> </td>";
	else
		echo "<td>Previous </td>";
	echo "<td><a href=\"se_report_list.php\">Back</a></td>";
	if( IsSet($next) && strlen( $next ) > 0 )
		echo "<td><a href=\"se_report_edit.php?report=".$next."\">Next</a> </td>";
	else
		echo "<td>Next </td>";
	echo "</tr>";
	echo "</table>";
