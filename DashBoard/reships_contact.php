<?php
	...
	if( $from < 0 or $to < 0 )
		...
		{
		$show = 9999;
		echo "<h2>Reshipments for orders shipped from $from to $to</h2>";
		}


	$clause = '1 = 1';
	if( $from > 0 )
		$clause = "DATE(original.or_shipped) >= '$from'";
	if( $to > 0 )
		$clause .= " AND DATE(original.or_shipped) <= '$to'";
	...
	if( $from > 0 )
		...
		if( $pos = strpos( $from, '-' ) )
			{
			$y = substr( $from, 0, $pos );
			$clause2 = "(original.or_archive_year >= $y";
			}
		}
	if( $to > 0 )
		{
	...
	if ($reship_result)
		{
		echo "<tr><th>Country</th><th>Order</th><th>Original</th><th>Email</th><th>Amount</th><th>Shipped</th><th>Reshipped</th><th>Notes</th></tr>";
		while( $row = mysql_fetch_assoc($reship_result) )
			...
			echo "<td>";
			$qn = "select * from shopsystem_order_notes where orn_or_id = ".$row['orOrID'];
			$rn = mysql_query( $qn );
