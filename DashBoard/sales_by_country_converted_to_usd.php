...
*/
	...
	mysql_query( "create temporary table foo as select cn_name, cn_id, count(orsi_box_number) as Boxes, sum(pr0_883_f) as Llamas, cast( 0.0  as double ) as Value from shopsystem_orders JOIN transactions on or_tr_id = tr_id JOIN shopsystem_order_sheets_items ON or_id = orsi_or_id JOIN shopsystem_product_extended_options ON orsi_stock_code = pro_stock_code JOIN shopsystem_products ON pr_id = pro_pr_id JOIN countries on cn_id = or_country where pr_deleted IS NULL and or_archive_year  = $year group by cn_name order by 1" );
	mysql_query( "create temporary table foo2 as select as_country, sum(as_sales / as_usd_rate) as as_sales from account_summary where as_year = $year group by as_country" );
	mysql_query( "update foo, foo2 set foo.Value = as_sales where cn_id = as_country" );

	echo "<h1>All sales for $year, Value converted into equivalent USD</h1>";
    display_query( "select cn_name, Boxes, Llamas, Value  as \$USD from foo",
