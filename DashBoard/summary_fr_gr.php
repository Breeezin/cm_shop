<?php
	$results = array();
	$Title = "Accounting Summary France and Greece Orders";
...
//	echo "<th>% cm</th>";
	...
	echo "<th>% Margin</th>";
	echo "<th>&euro; CM (before R&R)</th>";
	echo "<th>% Margin (before R&R)</th>";
	}
...
//	echo "<td align='right'>".number_format($row['cm_percent'], 1)."</td>";
	...
	echo "<td align='right'>".number_format($row['profit']+$row['refund_value']+$row['reship_value'], 2, '.', ',')."</td>";
	echo "<td align='right'>".number_format(($row['profit']+$row['refund_value']+$row['reship_value'])*100.0/$row['sales'], 1)."</td>";
	}
...
function show_query( $query )
	...
	if( IsSet( $want_year ) and !IsSet( $want_month ) )		// show all months for want_year
		...
		if( IsSet( $want_year ) and IsSet( $want_month ) )		// show all days for this month and year
			{
			$result = show_query("select as_day, sum(as_num_orders) as orders, sum(as_sales) as sales, sum(as_shipping_value) as shipping, sum(as_reship_value) as reship_value, sum(as_reship_boxes) as reship_boxes, sum(as_refund_value) as refund_value, sum(as_cm_value) as cm, sum(as_cm_value)*100/sum(as_sales) as cm_percent, sum(as_profit) as profit, sum(as_profit)*100/sum(as_sales) as margin_percent from account_summary join countries on as_country = cn_id where as_year = $want_year and as_month = $want_month and cn_id in (250,300) group by as_day order by as_day asc" );
			if ($result)
				{
				while( $row = mysql_fetch_assoc($result) )
					{
			...
			echo "<br/><br/><h2>Processed Orders France/Greece only</h2><br/>";
			$year_result = show_query("select as_year, sum(as_num_orders) as orders, sum(as_sales) as sales, sum(as_shipping_value) as shipping, sum(as_reship_value) as reship_value, sum(as_reship_boxes) as reship_boxes, sum(as_refund_value) as refund_value, sum(as_cm_value) as cm, sum(as_cm_value)*100/sum(as_sales) as cm_percent, sum(as_profit) as profit, sum(as_profit)*100/sum(as_sales) as margin_percent  from account_summary join countries on as_country = cn_id where as_year >= $first_year and as_year <= $this_year and cn_id in (250,300) group by as_year order by as_year, cn_id" );
			if ($year_result)
				{
			...
			$month_result = show_query("select sum(as_num_orders) as orders, sum(as_sales) as sales, sum(as_shipping_value) as shipping, sum(as_reship_value) as reship_value, sum(as_reship_boxes) as reship_boxes, sum(as_refund_value) as refund_value, sum(as_cm_value) as cm, sum(as_cm_value)*100/sum(as_sales), sum(as_profit) as profit, sum(as_profit)*100/sum(as_sales) as margin_percent from account_summary join countries on as_country = cn_id where as_year = $this_year and as_month = $this_month and cn_id in (250,300) order by cn_id" );
			...
			$day_result = show_query("select sum(as_num_orders) as orders, sum(as_sales) as sales, sum(as_shipping_value) as shipping, sum(as_reship_value) as reship_value, sum(as_reship_boxes) as reship_boxes, sum(as_refund_value) as refund_value, sum(as_cm_value) as cm, sum(as_cm_value)*100/sum(as_sales) as cm_percent, sum(as_profit) as profit, sum(as_profit)*100/sum(as_sales) as margin_percent  from account_summary join countries on as_country = cn_id where as_year = $this_year and as_month = $this_month and as_day = $this_day and cn_id in (250,300) order by cn_id" );
