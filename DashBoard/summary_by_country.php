...
//	echo "<td align='right'>".number_format($row['cm_percent'], 1)."</td>";
	...
	$this_month = date( 'm' );
	$this_day = date( 'd' );
	$this_script = 'summary_by_country.php';
	$now = time(NULL);
	...
	if( IsSet( $want_year ) and !IsSet( $want_month ) )		// show all months for want_year
		...
		if( IsSet( $want_year ) and IsSet( $want_month ) )		// show all days for this month and year
			{
			if( $NoUSA )
				$result = mysql_query("select cn_name, as_day, sum(as_num_orders) as orders, sum(as_sales) as sales, sum(as_shipping_value) as shipping, sum(as_reship_value) as reship_value, sum(as_reship_boxes) as reship_boxes, sum(as_refund_value) as refund_value, sum(as_cm_value) as cm, sum(as_cm_value)*100/sum(as_sales) as cm_percent, sum(as_profit) as profit, sum(as_profit)*100/sum(as_sales) as margin_percent from account_summary join countries on as_country = cn_id where as_year = $want_year and as_month = $want_month and cn_id != 840 group by cn_name, as_day order by as_day asc" );
			else
			...
			{
			echo "<br/><br/><h2>Processed Orders</h2><br/>";
			if( $NoUSA )
				$year_result = mysql_query("select cn_name, as_year, sum(as_num_orders) as orders, sum(as_sales) as sales, sum(as_shipping_value) as shipping, sum(as_reship_value) as reship_value, sum(as_reship_boxes) as reship_boxes, sum(as_refund_value) as refund_value, sum(as_cm_value) as cm, sum(as_cm_value)*100/sum(as_sales) as cm_percent, sum(as_profit) as profit, sum(as_profit)*100/sum(as_sales) as margin_percent  from account_summary join countries on as_country = cn_id where as_year >= $first_year and as_year <= $this_year and cn_id != 840 group by cn_name, as_year order by as_year, cn_id" );
			else
			...
			if( $NoUSA )
				$month_result = mysql_query("select cn_name, sum(as_num_orders) as orders, sum(as_sales) as sales, sum(as_shipping_value) as shipping, sum(as_reship_value) as reship_value, sum(as_reship_boxes) as reship_boxes, sum(as_refund_value) as refund_value, sum(as_cm_value) as cm, sum(as_cm_value)*100/sum(as_sales), sum(as_profit) as profit, sum(as_profit)*100/sum(as_sales) as margin_percent from account_summary join countries on as_country = cn_id where as_year = $this_year and as_month = $this_month and cn_id != 840 group by cn_name order by cn_id" );
			else
				$month_result = mysql_query("select cn_name, sum(as_num_orders) as orders, sum(as_sales) as sales, sum(as_shipping_value) as shipping, sum(as_reship_value) as reship_value, sum(as_reship_boxes) as reship_boxes, sum(as_refund_value) as refund_value, sum(as_cm_value) as cm, sum(as_cm_value)*100/sum(as_sales), sum(as_profit) as profit, sum(as_profit)*100/sum(as_sales) as margin_percent from account_summary join countries on as_country = cn_id where as_year = $this_year and as_month = $this_month group by cn_name order by cn_id" );
			...
			if( $NoUSA )
				$day_result = mysql_query("select cn_name, sum(as_num_orders) as orders, sum(as_sales) as sales, sum(as_shipping_value) as shipping, sum(as_reship_value) as reship_value, sum(as_reship_boxes) as reship_boxes, sum(as_refund_value) as refund_value, sum(as_cm_value) as cm, sum(as_cm_value)*100/sum(as_sales) as cm_percent, sum(as_profit) as profit, sum(as_profit)*100/sum(as_sales) as margin_percent  from account_summary join countries on as_country = cn_id where as_year = $this_year and as_month = $this_month and as_day = $this_day and cn_id != 840 group by cn_name order by cn_id" );
			else
				$day_result = mysql_query("select cn_name, sum(as_num_orders) as orders, sum(as_sales) as sales, sum(as_shipping_value) as shipping, sum(as_reship_value) as reship_value, sum(as_reship_boxes) as reship_boxes, sum(as_refund_value) as refund_value, sum(as_cm_value) as cm, sum(as_cm_value)*100/sum(as_sales) as cm_percent, sum(as_profit) as profit, sum(as_profit)*100/sum(as_sales) as margin_percent  from account_summary join countries on as_country = cn_id where as_year = $this_year and as_month = $this_month and as_day = $this_day group by cn_name order by cn_id" );
