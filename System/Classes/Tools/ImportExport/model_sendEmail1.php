<?php 
	...
	$email_body = "";
	$full_email_body = "";
	$recipients = array( "im@admin.com" );
	$full_recipients = array( "im@admin.com", "macbjorck@me.com" );

	// work out total profit since we last ran...

	$Q_last_ran = getRow( "select ds_timestamp as foo, unix_timestamp( ds_timestamp ) as foo2 from daily_skim order by ds_id desc limit 1" );
	$Q_yesterday = getRow( "select NOW() - INTERVAL 24 HOUR as foo" );
//	$Q_yesterday = getRow( "select NOW() - INTERVAL 4 MONTH as foo" );
	$Q_now = getRow( "select NOW() as foo, unix_timestamp() as foo2" );

	$now = $Q_now['foo'];
	$now_unix = $Q_now['foo2'];

	if( IsSet( $Q_last_ran['foo'] ) && $Q_last_ran['foo'] != NULL )
	{
		$last_ran = $Q_last_ran['foo'];
		$email_body .= "This utility last run at : $last_ran<br/>";
		$full_email_body .= "This utility last run at : $last_ran<br/>";
		$email_body .= "".(($now_unix-$Q_last_ran['foo2'])/3600)." hours ago<br/>";
		$full_email_body .= "".(($now_unix-$Q_last_ran['foo2'])/3600)." hours ago<br/>";
	}
	else
	{
		$last_ran = $Q_yesterday['foo'];
		$email_body .= "Utility first run, going back to $last_ran<br/>";
	}

	$Q_profit = getRow( "select sum(or_profit) as profit from shopsystem_orders where or_archive_year is null and (or_paid > '$last_ran' OR or_paid_not_shipped > '$last_ran')");

	$profit = $Q_profit['profit'];

	$Q_Fixed = getRow("SELECT * FROM invoice_configuration where ic_symbol = 'FIXED'" );

	$fixed = $Q_Fixed['ic_value'];

	$full_email_body .= "Profit : $profit<br/>";
	$full_email_body .= "Fixed cost : $fixed<br/>";

	if( $profit > $fixed )
	{
		$Q_comp = query( "select * from invoice_configuration, shopsystem_service_company where sic_id = ic_company" );

		while ($row = $Q_comp->fetchRow()) 
		{
//			$email_body .= "Skim for {$row['sic_name']} at ".($row['ic_value']*100)."% is $".(($profit-$fixed)*$row['ic_value'])."<br/>";
			$email_body .= "Transferir a {$row['sic_name']} $".(($profit-$fixed)*$row['ic_value'])."<br/>";
			$full_email_body .= "Drawings for {$row['sic_name']} at ".($row['ic_value']*100)."% is $".(($profit-$fixed)*$row['ic_value'])."<br/>";
			query( "insert into daily_skim (ds_company, ds_timestamp, ds_amount) values ({$row['sic_id']}, '$now', ".(($profit-$fixed)*$row['ic_value']).")");
			$recipients[] = $row['sic_email_address'];
		}
	}
	else
	{
		$email_body .= "Insufficient funds to cover skim since last run<br/>";
		$full_email_body .= "Insufficient funds to cover skim since last run<br/>";
	}


	$full_email_body .= "<br/><br/><br/><br/>Profit details<br/>";
	$Q_demo = query( "select or_recorded, or_tr_id, or_purchaser_firstname, or_purchaser_lastname, or_profit from shopsystem_orders where or_archive_year is null and (or_paid > '$last_ran' OR or_paid_not_shipped > '$last_ran')");

	while( $row = $Q_demo->fetchRow() )
	{
		extract( $row );
		$full_email_body .= "Order $or_tr_id at $or_recorded, from $or_purchaser_firstname $or_purchaser_lastname, profit is $$or_profit<br/>";
	}
	...
	foreach( $full_recipients as $recipient )
		$result = new Request('Email.Send',array(
						'to'	=>	$recipient, 
						'from'	=>	'webserver@acmerockets.com',
						'subject'	=>	"Daily Transfers with extra details",
