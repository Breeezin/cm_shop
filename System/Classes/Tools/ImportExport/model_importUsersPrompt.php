...
//		unlink($upload_temp);
		...
		while ($user = $Q_Users->fetchRow()) {
			...
			*/
            ...
            // a hack to ensure that it doesn't overwrite the existing groups
            if( array_key_exists('ImportUpdateKeepExistingGroups', $_REQUEST )
             || array_key_exists('ImportUpdateKeepExistingDetails', $_REQUEST ) ) {
                if($R_User = getRow ( "select * from users where us_email like '".escape($insertData['us_email'])."'")) {
                    if ( array_key_exists('ImportUpdateKeepExistingGroups', $_REQUEST ) ) {
                        $Q_Groups = query("select * from user_user_groups where uug_us_id={$R_User['us_id']}");
                        while ($row = $Q_Groups->fetchRow()){
                          if(in_array($row['uug_ug_id'],$this->ATTRIBUTES['user_groups']) === false)
                                $insertData['user_groups'] =
                                    array_merge(array($row['uug_ug_id']),$this->ATTRIBUTES['user_groups']);
                        }
                    }
                    if ( array_key_exists('ImportUpdateKeepExistingDetails', $_REQUEST ) ) {
                        $insertData = array_merge($R_User,$insertData);
                    }
                }
            }
