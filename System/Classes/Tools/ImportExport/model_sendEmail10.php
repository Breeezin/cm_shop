					...
					//						}
										...
										$Q_Insert = query("
											INSERT INTO shopsystem_order_sheets_items
												...
												orsi_qty, orsi_price, orsi_discount, orsi_total, orsi_or_id)
											VALUES
												($newOrderID, '".escape($row['oi_stock_code'])."', '".escape($prod['pr_name'])."',
												1, $price, $discount, $total, {$row['or_id']})
										");
								...
								where ors_id = $newOrderID order by orsi_or_id" );
						{
							while ( $row = $Q_OrderSheetItems->fetchRow() )
							{
								...
								if( strcmp( $order_id, $db.'-'.$row['or_tr_id'] ) )
								{
									...
									$delivery_street_address = $shippingDetails['ShippingDetails']['0_50A1'];
									$delivery_city = $shippingDetails['ShippingDetails']['0_50A2'];
									$delivery_state = $state;
									$delivery_country = $country;
									$delivery_postcode = $shippingDetails['ShippingDetails']['0_B4C0'];
									$customers_telephone = $shippingDetails['ShippingDetails']['0_B4C1'];
								...
								$pdf->CellNextLine( "Arial", "", "12", "L", 0, 0, 0, $row['orsi_qty']." ".$row['orsi_pr_name'] );
