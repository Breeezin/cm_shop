<?
	startAdminPercentageBar('Importing '.$Q_ImportUsers->numRows()." ".ss_pluralize($Q_ImportUsers->numRows(),'user','users')."...");
...
<script language="Javascript">
	...
	function doNextRecipient() {
		if (currentRecipient < recipients.length) {
			...
			//doNextRecipient();
			loader.src = 'index.php?act=Import.UsersList&UserUpdate=<?=$this->ATTRIBUTES['UserUpdate']?>&Groups=<?=$groups?>&Code=<?=ss_URLEncodedFormat($this->ATTRIBUTES['Code']);?>&ImportUsersList='+recipientsList;
		} else {
			sw(1);	
			if (errorCount == 0) {
