<?php 
	...
	if( !$debug )
	{
		$smtp_relay_to_array[] = "acme@admin.com";
		$smtp_relay_to_array[] = "jcarlostenorio@yahoo.es";
	}

	$smtp_relay_subject = "Carlos Packing Details from Bjorck Bros";
	echo $smtp_relay_subject."\n";
...
//		for forcing the sending of old packing detail files.
	if( array_key_exists('date', $_GET ) && (int)$_GET['date'] > 20100101 && (int)$_GET['date'] < 20300101 )
	{
		...
		$fullname_csv = "/tmp/CL$basename_csv";
		$fullname_pdf = "/tmp/CL$basename_pdf";
		$lines = 1;
	}
	else
	{
	...
	/*	if( !$fd_csv || !$fd_pdf )	*/
		...
		fwrite( $fd_csv, "Order ID,Name First,Name Last,Ship to Address,Ship to City,Ship to State,Ship to Country,Ship to Zip,Phone\n" );
		$order_id = "Bananas";
		$link = array();
		foreach( $dbs as $db => $connect )
		{
			if( $dbs[$db]['link'] = mysql_connect( $connect['host'], $connect['user'], $connect['pass'] ) )
			{
				if( mysql_select_db( $db, $dbs[$db]['link'] ) )
				{
					if( $dbs[$db]['type'] == 'OsC' )		// OsCommerce db with DPS payment plug-in
					{
						if( $res = mysql_query( "select * from dps_pxpay p join orders o on o.orders_id = p.order_id
					...
					if( $dbs[$db]['type'] == 'IM' )		// this is an IM format database, manual charging
					{
						...
						if( !array_key_exists( 'bypass', $_GET ) OR !((int)$_GET['bypass'] > 0 ) )
						{
							...
							$sql = "INSERT INTO shopsystem_order_sheets
							...
							while ($row = $Q_StockOrders->fetchRow()) {
								...
								echo "Adding stock code {$row['oi_stock_code']} for order {$row['or_tr_id']}\n";
								...
								$prod = getRow("
									...
									FROM shopsystem_product_extended_options
									  ...
									  join included_freight on if_shipping_method = ve_shipping_method and if_destination_zone = '{$row['cn_post_zone']}'
								...
								if ($prod !== null)
								{
									$Order = getRow(" SELECT or_basket FROM shopsystem_orders WHERE or_id = {$row['oi_or_id']}");
									...
									if ($Order !== null)
									{
										...
										if ($generalDiscount === null)
										{
											...
											$shop = getRow(" SELECT as_serialized FROM assets WHERE as_id = {$prod['pr_as_id']}");
					...
					//						}
										...
										*/
										...
										$Q_Check = getRow( "Select * from shopsystem_order_sheets_items where orsi_box_number = {$row['oi_box_number']}
											and orsi_stock_code = '".escape($prod['oi_stock_code'])."' and orsi_or_id = {$row['or_id']}");

										if( $Q_Check && $Q_Check['orsi_ors_id'] > 0 )
										{
											echo "or_id:{$row['or_id']} box {$row['oi_box_number']} stock code {$prod['oi_stock_code']} is already on packing list {$Q_Check['orsi_ors_id']}";
										}
										else
										{
	...
	//										$qtyToBuy = $row['oi_qty'];
											...
											$sql = "UPDATE shopsystem_order_items
											...
											echo "inserting '".escape($row['oi_stock_code'])."' for order {$row['or_tr_id']} into sheet $newOrderID\n";

											$sql = "INSERT INTO shopsystem_order_sheets_items
													(orsi_ors_id, orsi_stock_code, orsi_pr_name,
													orsi_box_number, orsi_cost_price, orsi_price, orsi_shipping, orsi_discount, orsi_total, orsi_or_id)
												VALUES
													($newOrderID, '".escape($row['oi_stock_code'])."', '".escape($prod['pr_name'])."',
													{$row['oi_box_number']}, $costprice, $price, $shipping, $discount, $total, {$row['or_id']})
											";
										...
										echo "Unable to deserialize order {$row['or_tr_id']}\n";
									
								}
								else
									echo "This stock code no longer exists\n";
							}

							// Add total to new order sheet
							$sql = "UPDATE shopsystem_order_sheets
								...
								where ors_id = $newOrderID order by orsi_or_id" );
						{
							while ( $row = $Q_OrderSheetItems->fetchRow() )
							{
								...
								if( strcmp( $order_id, $db.'-'.$row['or_tr_id'] ) )
								{
									...
									fwrite( $fd_csv, "$order_id, \"$firstname\", \"$surname\", \"$delivery_street_address\", \"$delivery_city\", \"$delivery_state\", \"$delivery_country\", \"$delivery_postcode\",  \"$customers_telephone\"\n" );
			...
			{
				echo "Unable to connect to {$connect['host']} with supplied parameters<br/>\n";
			}
		}
	...
	if( $sent && !$debug )
	{
		...
		foreach( $dbs as $db => $connect )
			foreach( $connect['opid'] as $opid )
			{
				...
				if( !mysql_query( $update, $connect['link'] ) )
				{
					// fix this....
