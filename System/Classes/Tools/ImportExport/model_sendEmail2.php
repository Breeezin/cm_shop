<?php 
	...
	$email_body = "";
	///$recipients = array( "im@admin.com", "luisnbusuario@gmail.com" );
	$recipients = array( "acme@admin.com" );

	// work out total profit since we last ran...

	$Q_sum = query( "select ds_company, sic_name, sic_email_address, sum( ds_amount) as weeksum from daily_skim, shopsystem_service_company where sic_id = ds_company and ds_timestamp > NOW() - INTERVAL 1 WEEK group by ds_company, sic_name, sic_email_address" );

	while ($row = $Q_sum->fetchRow()) 
	{
		$email_body .= "Total invoice for {$row['sic_name']} is $".number_format($row['weeksum'], 2)."<br/>";
//		echo "Total invoice for {$row['sic_name']} is {$row['weeksum']}<br/>";
		$recipients[] = $row['sic_email_address'];
	}

	$email_body .= "Click <a href='http://www.acmerockets.com/index.php?act=ShopSystem_ServiceInvoiceAdministration.Auto'>here</a> to automatically create these invoices<br/>";
//	echo "Click <a href='http://www.acmerockets.com/index.php?act=ShopSystem_ServiceInvoiceAdministration.New'>here</a> to automatically create these invoices<br/>";
						...
						if (!$found) {
							$product = getRow("
								SELECT pr_id,pr_short,pr_name,pro_uuids, pro_stock_code,
									pro_price,pro_special_price,pro_member_price,pr_ve_id
