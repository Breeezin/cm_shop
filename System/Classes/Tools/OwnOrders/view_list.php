...
<TABLE WIDTH="100%" ALIGN="CENTER" BORDER="0" CELLSPACING="0" CELLPADDING="2">
	<TR>	
		...
		<FORM ACTION="index.php?act=OwnOrdersAdministration.List&as_id=<?=$this->ATTRIBUTES['as_id']?>" METHOD="POST">
		...
		<?php if (ss_optionExists('Shop Advanced Ordering')) { ?>
			...
			<?php if( array_key_exists( 'User', $_SESSION )
					...
					{
			?>
			<INPUT type="button" name="logout" value="New Order" onclick="openwindow('<?=ss_JSStringFormat(ss_withTrailingSlash($GLOBALS['cfg']['secure_server']));?>Shop_System/Service/ClientsNewOrder/ExistingClient/<?print $_SESSION['User']['us_id']?>?AccessCode=<?print(ss_URLEncodedFormat($accessCode));?>&<?=ss_getHashMeInURL();?>','NewOrder');">
			<?php } ?>
		<?php } ?>
		<?php 
		...
		<!--- Search Options Selector --->
			...
			<TD ALIGN="RIGHT">
				...
				<TABLE WIDTH="100%"  CELLPADDING="2" CELLSPACING="0" BORDER="0">
					...
					<TD ALIGN="RIGHT"><INPUT SIZE="10" TYPE="TEXT" VALUE="<?php print ss_HTMLEditFormat($this->ATTRIBUTES['ShippingDateKeyword']) ?>" NAME="ShippingDateKeyword"> (yyyy-mm-dd)</TD>
					</TR>
        ...
        { 
			...
			foreach ($this->tableDisplayFields as $displayField) 
            {
				...
				if ($this->tableTimeStamp == $displayField) 
                {
                    ...
                    {
						$value = formatDateTime($row[$displayField], "j M y");
					}
                    else 
                    {
						$value = "&nbsp;";	
                ...
                {	
					$value = ss_getTrasacationRef($row['tr_id']);
				} else if ($displayField == 'tr_currency_link')
                {
					$value = $row['cn_currency_symbol'].$row['tr_total'].' '.$row["cn_currency_code"];
				} else if ($displayField == 'or_profit')
                {
//					$value = "&nbsp;";
					//if ($row['or_shipped'] !== null) {
						$value = $row['cn_currency_symbol'].ss_decimalFormat($row['or_profit']).' '.$row["cn_currency_code"];
//					}
				} else if ($displayField == 'TrSelect')
                {
					$value = "<input type='checkbox' name='row_selected' value='".$row['tr_id']."'>";
				...
				if ($displayField == 'tr_timestamp' and ss_optionExists('Shop Acme Rockets'))
                {
                    ...
                    $mouseOverDisplay = "<br/>Order Details<br/><table width=300>";
...
//                        for ($qty=0; $qty < $entry['Qty']; $qty++)
			    ...
			    $mouseOverDisplay .= "<tr><td align=\'LEFT\'>".$entry['Product']['pr_name'];
                            ...
                            if (strlen($entry['Product']['Price']))
                                $mouseOverDisplay .= "<td align=\'RIGHT\'> <b>".$row['cn_currency_symbol'].ss_decimalFormat($entry['Product']['Price'])."</b></td>";
                    ...
                    print( "<TD ALIGN=\"LEFT\"><a href=\"javascript:void(0);\" onMouseOver=\"overlib('"
                        .$mouseOverDisplay
			...
			{
				print( '<img src="System/Classes/AssetTypes/ShopSystemAsset/ShopSystem_OrdersAdministration/Templates/Images/shipped.gif"></img>' );
			}
			if( strlen($row['or_paid'] ) )
			{
				print( '<img src="System/Classes/AssetTypes/ShopSystemAsset/ShopSystem_OrdersAdministration/Templates/Images/paid.gif"></img>' );
			}
