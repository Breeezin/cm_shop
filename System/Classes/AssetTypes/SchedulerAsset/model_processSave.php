<?php
	...
	$assetID = $asset->getID();	
	$initAdd = false;	
	$Q_Scheduler = getRow("SELECT * FROM Events LIMIT 1");
	if (!is_array($Q_Scheduler)) {
		$Q_AddNewRow = query("INSERT INTO Events (EvID) VALUES (0)");
		$initAdd = true;	
		$Q_Scheduler = getRow("SELECT * FROM Events LIMIT 1");
	}
	...
	foreach($fieldsArray as $fieldDef) {
		...
		$dbFieldName = 'Ev'.$fieldDef['uuid'];		
		
		// is not existing 
		if (strlen($fieldDef['uuid']) AND !array_key_exists("$dbFieldName", $Q_Scheduler)) {
			// add a new column called "Us{UUID}"
			$Q_AlterTable = query("ALTER TABLE  Events ADD $dbFieldName LONGTEXT");								
		}
	...
	foreach($Q_Scheduler as $key => $value) {
		if (array_search($key, $fixedUserColumnNames) === false) {		
			if (strpos($key,'_')) {			
				$Q_AlterTable = query("ALTER TABLE  Events DROP $key");
	...
	if ($initAdd) {
		$Q_DeleteTheNewRow = query("DELETE FROM Events WHERE EvID = 0");		
	}
