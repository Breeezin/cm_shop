<?php
	 ...
	 && ( strlen( $_SESSION['User']['us_password'] ) > 0 ) )
	{
		...
		$expire = getRow("
			SELECT MIN(up_expires) AS Expiry FROM shopsystem_user_points
			WHERE up_us_id = {$usID}		
				AND up_used IS NULL
				AND up_expires > CURDATE()
		");
...
//							$price = escape($this->formatPrice('display', $price));
		...
		$this->useTemplate('WelcomeService',$data);
		return;
	}
	else
	{
		$nothing = query("select NULL from users where 1=0");
		$data = array(
			'AssetPath'			=>	$assetPath,
			'WishList'			=>	$nothing,
