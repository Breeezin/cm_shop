<?php
    ...
    if (count($item['Children']) and $displayChildren and ss_optionExists('Horizontal Subnav')){
        $subnavMouseOver = 'showSubNav()';
        $linkMouseOver = '';
        $subnavMouseOut = 'hideSubNav()';
        $linkMouseOut = '';
    }
	...
	if (strlen($this->ATTRIBUTES["AST_MENU_{$stylePrefix}ROLLOVERCLASS"]) > 0) {
		$cellMouseOver = "ONMOUSEOVER=\"this.className='{$this->ATTRIBUTES["AST_MENU_{$stylePrefix}ROLLOVERCLASS"]}';$subnavMouseOver";
		$cellMouseOut = "ONMOUSEOUT=\"this.className='$cellClass';$subnavMouseOut";
		if (!$selectedItem) {
			if (array_key_exists($item['as_id'], $currentAssetTree)) {					
