<?php
    ...
    //done for James Dunlop/Kovacs - just shows/hides a second menu
    $subnavMouseOver = '';
    $subnavMouseOut = '';
    if (count($item['Children']) and $displayChildren and ss_optionExists('Horizontal Subnav')){
        $subnavMouseOver = "showSubNav('". $item['as_name'] . "')";
        $linkMouseOver = '';
        $subnavMouseOut = "hideSubNav('". $item['as_name'] . "')";
        $linkMouseOut = '';
    }

    //allows a different class to be pulled in for parents with children
    $hasChildrenClass = $item['HasChildren'] ? ' ChildMenuCell' : '';

	$cellMouseOver = '';
	$cellMouseOut = '';
	$cellClick = '';
	if (strlen($this->ATTRIBUTES["AST_MENU_{$stylePrefix}ROLLOVERCLASS"]) > 0) {
		$cellMouseOver = "ONMOUSEOVER=\"this.className='{$this->ATTRIBUTES["AST_MENU_{$stylePrefix}ROLLOVERCLASS"]}$hasChildrenClass';$subnavMouseOver $linkMouseOver";
		$cellMouseOut = "ONMOUSEOUT=\"this.className='$cellClass$hasChildrenClass';$subnavMouseOut$linkMouseOut";
		if (!$selectedItem) {
			if (array_key_exists($item['as_id'], $currentAssetTree)) {
				$cellClass = $this->ATTRIBUTES["AST_MENU_{$stylePrefix}ROLLOVERCLASS"];
				$cellMouseOut = "ONMOUSEOUT=\"this.className='$cellClass$hasChildrenClass'; $linkMouseOut";
				$selectedItem = true;
			} else if($currentAsset == $item['as_id']){
				$cellClass = $this->ATTRIBUTES["AST_MENU_{$stylePrefix}ROLLOVERCLASS"];
				$cellMouseOut = "ONMOUSEOUT=\"this.className='$cellClass$hasChildrenClass'; $linkMouseOut";
    ...
    //briar moved this down so the onmouseover wasn't appended for the previous bit yet
