...
<TABLE WIDTH="100%" ALIGN="CENTER" BORDER="0" CELLSPACING="0" CELLPADDING="2">
	<TR>
		...
		<TD ALIGN="LEFT" valign="baseline">
			<a href='index.php?act=CustomerAdministration.List' target='_blank'>Manage Customers</a>
		</TD>
	...
	if ($hasRows) {
		...
		while(($row = $result->fetchRow()) && ($currentRow < $startRow+$this->ATTRIBUTES['RowsPerPage'])) { 
			...
			foreach ($this->tableDisplayFields as $displayField) {
				...
				if( $displayField == 'cin_cp_id' )
					$value = getField( "Select cp_name from customer where cp_id = ".((int)$value ) );
