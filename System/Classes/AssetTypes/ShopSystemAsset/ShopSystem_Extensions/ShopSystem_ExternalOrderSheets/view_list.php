...
<input type="button" value="Generate new Order Sheet" onclick="document.location='index.php?act=ShopSystem.ExternalOrderList&amp;vendor=<?=$this->ATTRIBUTES['vendor']?>';">
<br />
<br />
<input type="button" value="Customs Report" onclick="window.open('index.php?act=shopsystem_order_sheets.Customs2&vendor=<?=$this->ATTRIBUTES['vendor']?>');">
<br />
<input type="button" value="Inventory Report" onclick="window.open('index.php?act=shopsystem_order_sheets.Inventory&vendor=<?=$this->ATTRIBUTES['vendor']?>');">
			...
			<TD ALIGN="RIGHT">
				...
				<TABLE WIDTH="100%"  CELLPADDING="2" CELLSPACING="0" BORDER="0">
					...
					<select name="jumperSelect" onChange="jumper(this)">
						<?php while(($vrow = $vendors->fetchRow()) ) { ?>
						<option value="index.php?act=shopsystem_order_sheets.List&vendor=<?php echo $vrow['ve_id'];?>"
						<?php if( $this->ATTRIBUTES['vendor'] == $vrow['ve_id'] )
							echo " selected"; ?>
	...
	if ($hasRows) {
		...
		while(($row = $result->fetchRow()) && ($currentRow < $startRow+$this->ATTRIBUTES['RowsPerPage'])) { 
			...
			print('<TD ALIGN="RIGHT" VALIGN="BOTTOM"><SELECT NAME="jumperSelect" onChange="jumper(this)"><OPTION VALUE="#">Manage</OPTION>');
				...
				print("<OPTION VALUE=\"{$script_name}?act={$this->prefix}%2EView&{$this->tablePrimaryKey}={$row[$this->tablePrimaryKey]}&BreadCrumbs=".ss_URLEncodedFormat($breadCrumbs)."&BackURL=".ss_URLEncodedFormat($rfa));
					
				if ($this->tableAssetLink !== null or $this->assetLink !== null) {
					print("&as_id={$this->assetLink}");
				}
				print("\">View as picking list</OPTION>");

				// hack in an alternate view
				print("<OPTION VALUE=\"{$script_name}?act={$this->prefix}%2EViewPacking&{$this->tablePrimaryKey}={$row[$this->tablePrimaryKey]}&BreadCrumbs=".ss_URLEncodedFormat($breadCrumbs)."&BackURL=".ss_URLEncodedFormat($rfa));
					
				if ($this->tableAssetLink !== null or $this->assetLink !== null) {
					print("&as_id={$this->assetLink}");
				}			
				print("\">View as MAIN packing list</OPTION>");

				if( $dq = query( 'select * from users where us_vendor_distributor = '.((int)$this->ATTRIBUTES['vendor']) ) )
				{
					while( $distributor = $dq->fetchRow( ) )
					{
						// hack in an alternate view
						print("<OPTION VALUE=\"{$script_name}?act={$this->prefix}%2EViewPacking&{$this->tablePrimaryKey}={$row[$this->tablePrimaryKey]}&Distributor={$distributor['us_id']}&BreadCrumbs=".ss_URLEncodedFormat($breadCrumbs)."&BackURL=".ss_URLEncodedFormat($rfa));
							
						if ($this->tableAssetLink !== null or $this->assetLink !== null) {
							print("&as_id={$this->assetLink}");
						}			
						print("\">View {$distributor['us_0_50A2']} packing list</OPTION>");

						print("<OPTION VALUE=\"{$script_name}?act={$this->prefix}%2ECustoms3&{$this->tablePrimaryKey}={$row[$this->tablePrimaryKey]}&Distributor={$distributor['us_id']}&BreadCrumbs=".ss_URLEncodedFormat($breadCrumbs)."&BackURL=".ss_URLEncodedFormat($rfa));
							
						if ($this->tableAssetLink !== null or $this->assetLink !== null) {
							print("&as_id={$this->assetLink}");
						}			
						print("\">View {$distributor['us_0_50A2']} customs report</OPTION>");

					}
				}

				// Print the manage cell
				print("<OPTION VALUE=\"{$script_name}?act={$this->prefix}%2ECustoms&{$this->tablePrimaryKey}={$row[$this->tablePrimaryKey]}&BreadCrumbs=".ss_URLEncodedFormat($breadCrumbs)."&BackURL=".ss_URLEncodedFormat($rfa));
					
				if ($this->tableAssetLink !== null or $this->assetLink !== null) {
					print("&as_id={$this->assetLink}");
				}
				print("\">View Post Office report</OPTION>");

				// hack in an alternate view
				print("<OPTION VALUE=\"{$script_name}?act={$this->prefix}%2EManageLocations&{$this->tablePrimaryKey}={$row[$this->tablePrimaryKey]}&BreadCrumbs=".ss_URLEncodedFormat($breadCrumbs)."&BackURL=".ss_URLEncodedFormat($rfa));
					
				if ($this->tableAssetLink !== null or $this->assetLink !== null) {
					print("&as_id={$this->assetLink}");
				}			
				print("\">Manage Product Locations</OPTION>");
