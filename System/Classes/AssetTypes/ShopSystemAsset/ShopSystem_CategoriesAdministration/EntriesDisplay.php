...
<!--
	...
	function appearSelected(theForm) {		
		var ids = '';
		var not_ids = '';
		var deleted = null;		
	
		var checkField = 'Appears_<?=$this->tablePrimaryKey?>';
		...
		if (theForm[checkField].length) {
			...
			for(var i=0; i < theForm[checkField].length; i++) {
				if (theForm[checkField][i].checked) {	
					...
					not_ids += theForm[checkField][i].value;					
				}
			}
		} else {
			...
			//alert(ids);
			document.location  = 'index.php?act=<?=$this->prefix?>%2EAppears&<?=$this->tablePrimaryKey?>='+ids+'&UnCaID='+not_ids+'&as_id=<?=$this->assetLink?>&BackURL=<?=ss_URLEncodedFormat($rfa)?>';
