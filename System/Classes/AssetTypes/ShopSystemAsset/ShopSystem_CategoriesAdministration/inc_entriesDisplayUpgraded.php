	...
	if ($hasRows) {
		...
		while(($row = $result->fetchRow()) && ($currentRow < $startRow+$this->ATTRIBUTES['RowsPerPage'])) { 		
			...
			foreach ($this->tableDisplayFields as $displayField) {
				...
				if ($displayField =='ca_appears_in_menu') {
					$checked = '';
					if (strlen($row[$displayField])) {
						$checked = 'checked';
					}
					$value = "<input type=\"checkbox\" $checked name=\"Appears_{$this->tablePrimaryKey}\" value=\"{$row[$this->tablePrimaryKey]}\">";
					
				} else if ($this->tableTimeStamp == $displayField) {
...
<TABLE WIDTH="100%" ALIGN="CENTER" BORDER="0" CELLSPACING="0" CELLPADDING="2">
	<TR>
		...
		<?php if (ss_optionExists('Show Shop Category Menu')) { ?>
		<TD ALIGN="LEFT" valign="baseline">
            <INPUT TYPE="button" NAME="select" VALUE="Select All" onclick="selectAll(this.form)">
		</TD>
		<TD ALIGN="LEFT" valign="baseline">
            <INPUT TYPE="button" NAME="appearsInMenu" VALUE="Save Category Menu Setting" onclick="appearSelected(this.form)">
		</TD>
		<?php } ?>
