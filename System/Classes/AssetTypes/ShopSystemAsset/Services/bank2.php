...
// rural redirection page...

$ACK_URL = "{$secureSite}$assetPath/Service/Completed/tr_id/{$this->ATTRIBUTES['tr_id']}/tr_token/{$this->ATTRIBUTES['tr_token']}/us_id/$usID";
$NACK_URL = "{$normalSite}Members";
$POST_URL = "https://tpv01.cajarural.com/nuevo_tpv/tpv/jsp/tpvjp_validaComercio.jsp";		// test
$CIP = '98715610546';//11111111
$MerchantID = '249059684';//249059684

$ChargeCurrency = '978';		// euros
$TerminalID='001';
$fecha = strftime( "%y%m%d%H%M%S" );
$importe = (int)$totalPrice*100;

$sha = new SHA;
//$message = $_GET['fecha'].$_GET['cip'].$_GET['importe'].$_GET['moneda'].$_GET['numpedido'];
$message = $fecha.$CIP.$importe.$ChargeCurrency.$this->ATTRIBUTES['tr_id'];
...
<input type="hidden" name="moneda" value="<?=$ChargeCurrency?>">
<input type="hidden" name="numpedido" value="<?=$this->ATTRIBUTES['tr_id']?>">
<input type="hidden" name="fuc" value="<?=$MerchantID?>">
<input type="hidden" name="idterminal" value="<?=$TerminalID?>">
<input type="hidden" name="idioma" value="5">
<input type="hidden" name="fecha" value="<?=$fecha?>">
<input type="hidden" name="cip" value="<?=$CIP?>">
<input type="hidden" name="url" value="<?=$ACK_URL?>">
