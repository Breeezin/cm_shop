<?php 
	...
	foreach($formFields as $index => $formField) {				
		...
		$defineFieldsJS .= "FieldSet[$index] = new Field('".ss_JSStringFormat($formField['title'])."','".ss_JSStringFormat($formField['url'])."','".ss_JSStringFormat($formField['description'])."','".ss_JSStringFormat($formField['link'])."', '".ss_JSStringFormat($formField['uuid'])."');\n";
...
<!--
	...
	NextUID = 1;
	uids = new Array(<?php print $uidJS?>);
	...
	function openImageFolder() {
		...
		var url = 'index.php?act=AssetSelector.SharedImageDisplay&OnClick=opener.document.AssetForm.FieldURL.value=selectedImage;opener.loadImage();&as_id=<?php print $asset->getID()?>' + selectedImage;
		var newwin = window.open(url, 'sharedImageDirectory','width=590,height=320');
	}
	function Field(title, url, description, link, uuid) {
		this.title = title;		
		this.url = url;
	...
	function New() {
		...
		FieldSet[FieldSet.length] = new Field('Untitled','','','','');
	...
	function loadImage() {
		...
		if (theForm.FieldURL.value.length) {
			...
			document.images.galShowImage.src = "<?php print $directory?>" + theForm.FieldURL.value;			
		} else {
	...
	function DumpFieldSet() {
		...
		document.forms.AssetForm.<?php print $this->fieldPrefix?>FORM.value = formDef;
	}
...
//-->
	...
	<INPUT TYPE="HIDDEN" NAME="<?php print $this->fieldPrefix;?>FORM"  VALUE="<?=ss_JSStringFormat($formFieldValue)?>">
			...
			<FIELDSET>
				...
				<TABLE WIDTH="100%">
					<TR>
						...
						<TD>
						<?php print $this->fieldSet->displayField("AST_GALLERY_THUMBNAIL_WIDTH");?>						
						</TD>
					</TR>
					...
					<TR>
						...
						<TD>
						<?php print $this->fieldSet->displayField("AST_GALLERY_THUMBNAIL_HEIGHT");?>						
						</TD>
					</TR>
					...
					<TR>
						...
						<TD>
						<?php print $this->fieldSet->displayField("AST_GALLERY_IMAGES_PER_ROW");?>						
						</TD>
					</TR>
					<TR>
						<TD>Rows Per Page :<BR>
							<SMALL>(Leave blank to display all gallery images on a single page)</SMALL>
						</TD>
						<TD>	
						<?php print $this->fieldSet->displayField("AST_GALLERY_ROWS_PER_PAGE");?>													
						</TD>
					</TR>					
					<TR>
						<TD>Popup Width :</TD>
						<TD>
						<?php print $this->fieldSet->displayField("AST_GALLERY_POPUP_WIDTH");?>						
						</TD>
					</TR>
					<TR>
						<TD>Popup Height :</TD>
						<TD>
						<?php print $this->fieldSet->displayField("AST_GALLERY_POPUP_HEIGHT");?>						
						</TD>
					</TR>
				</TABLE>	
			</FIELDSET>				
