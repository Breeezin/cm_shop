...
//	ss_log_message( "Actually sending newsletters to ".$Q_Recipients->numRows()." people" );
...
//		ss_log_message( "Sending to ".$recipient['us_email'] );
		...
		if (!$recipient['us_html_email']) {
            ...
            $mailerNoHTML = new htmlMimeMail();
            $mailerNoHTML->setFrom($newsletter['nl_sender_email']);
            $mailerNoHTML->setSubject($newsletter['nl_subject']);
            $mailerNoHTML->setText($textMessage);

            $mailerNoHTML->send(array($recipient['us_email']));
//            echo "Sent NoHTML";
//            ss_DumpVar( $mailerNoHTML );
//            echo "\n => ".$recipient['us_email'];
		} else {
            unset( $mailerHTML );
            $mailerHTML = new htmlMimeMail();
            $mailerHTML->setFrom($newsletter['nl_sender_email']);
            $mailerHTML->setSubject($newsletter['nl_subject']);
...
//            echo "================\n".$htmlMessage."\n=====================\n";
    ...
    //					ss_log_message( "adding image ".$imagePath." name ".$imageNames[$imagePath]." type ".$imageImg->mimeType );
    					$mailerHTML->addHtmlImage($mailerHTML->getFile($imagePath), $imageNames[$imagePath], $imageImg->mimeType);
    				}
                }
    ...
    //					ss_log_message( "adding file ".$filePath );
    					$mailerHTML->addHtmlImage($mailerHTML->getFile($filePath), $fileNames[$filePath], 'text/css');
    				}
                }
				...
				foreach (array('/<img[^>]* src="([^"]+)"[^>]*>/is','/background="([^"]+)"/is','/<link[^>]* href="([^"]+\.css)"[^>]*>/is') as $regex) {
					...
					for ($i=count($matches[0])-1; $i>=0; $i--) {
						...
						$htmlMessage = substr_replace($htaddSlashesmlMessage,$imagePath,$matches[1][$i][1],strlen($matches[1][$i][0]));	
					}
				}
			}
			...
			$mailerHTML->setHtml($htmlMessage,$textMessage);
            $mailerHTML->is_built = false;
            unset( $mailerHTML->output );
            $mailerHTML->send(array($recipient['us_email']));

//            echo "Sent HTML\n\======================================n";
//            ss_DumpVar( $mailerHTML );
//            echo "\n===================================\n => ".$recipient['us_email'];
		}
