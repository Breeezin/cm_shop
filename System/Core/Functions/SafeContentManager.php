<?php
	...
	function ss_getDiskSpaceUsage ($isReturnAll = true) {
		...
		$cmdDu = "/usr/bin/du -cs {$dirPath}Custom | /usr/bin/grep total";
		...
		$cmdDu = "/usr/bin/du -cs {$dirPath}Custom/ContentStore/ImportExport | /usr/bin/grep total";		
	    $cm_result = exec($cmdDu);
	...
	*/
	...
	function ss_optionExists($option) {
		...
		if (array_key_exists($option, $GLOBALS['cfg']['Options']))
