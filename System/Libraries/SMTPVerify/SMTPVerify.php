          ...
          if ($this->sock) {
           ...
           if($code != '220') {
            ...
            foreach($users as $user) {
                ...
                $results[$user.'@'.$domain.'.error'] = $reply;
                }
                continue;
           }
           ...
           fclose($this->sock);
		...
		else
			{
				foreach($users as $user) {
					$results[$user.'@'.$domain] = false;
					$results[$user.'@'.$domain.'.error'] = "Unable to connect to mail server";
					}
			  $this->debug("no socket");
			}
        }
        return $results;
 }
 ...
 function queryMX($domain) {
        ...
        if (function_exists('getmxrr')) {
                if( !getmxrr($domain, $hosts, $mxweights) )
					$this->debug("using getmxrr($domain) failed");
        } else {
                // windows, we need Net_DNS
                require_once 'Net/DNS.php';
				$this->debug("not using getmxrr");
