...
//
// $Id: spell-check-ui.js,v 1.3 2003/09/28 12:10:53 mishoo Exp $
...
function saveClicked() {
	if (modified) {
		...
		alert('Changes sent to document.');
	} else {
...
function replaceWord(el) {
	...
	thisOneModified = (el.innerHTML != replacement);
	if (el) {
...
function esacpeString(s) {
	var temp = "" + s;
	for (var i=0; i < temp.length; i++) {
		if (s.charCodeAt(i) > 127) {
			//temp = "" + (temp.substring(0, i) + '&#'+s.charCodeAt(i)+';' + temp.substring((i + 1), temp.length));
			temp = replaceChars(temp,s.charAt(i),'&#'+s.charCodeAt(i)+';');
			//alert('pos: '+i+' '+s.charCodeAt(i)+' '+s.charAt(i));
		}
	}
	return temp;
}
...
function initDocument() {
	...
	// weird quotes seem to screw up the spellchecker, so escape them.
	field.value = esacpeString(field.value);
