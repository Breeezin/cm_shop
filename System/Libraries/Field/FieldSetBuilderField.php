	...
	class FieldSetBuilderField extends TextField {
		...
		function display($verify = FALSE, $formName = NULL, $multi = FALSE, $class = NULL) {
			...
			foreach($formFields as $index => $formField) {				
                ...
                // this can cause errors if set to ""
                if (strlen($formField['prefixed']) == 0)
                     $formField['prefixed'] = 0;
				ss_paramKey($formField,'comments','');
				...
				$defineFieldsJS .= "{$name}_FieldSet[$index].comments = '".ss_JSStringFormat($formField['comments'])."';\n";
					...
					if (is_array($formField['options'])) {
						for ($i = 0; $i < count($formField['options']);$i++) {
							if (is_array($formField['options'][$i]) and count($formField['options'][$i])) {
								$defineFieldsJS .= "{$name}_FieldSet[$index].options[{$i}] = new {$name}_Option('".ss_JSStringFormat($formField['options'][$i]['name'])."','{$formField['options'][$i]['uuid']}');\n";
							}
						}
					}
				} else {				
...
*/			
			$cmDataFieldhtml = '';			
			$cmDataFieldLoadJS = '';			
			$cmDataFieldStoreFunctionJS = '';			
			$cmDataSelectFieldhtml = '';			
			$cmDataOptionFieldLoadJS = '';			
			$cmDataOptionFieldStoreFunctionJS = '';			
			if (ss_HasPermission('IsDeployer') and ss_optionExists('Change Field Set Builder UUID')){
				$cmDataFieldhtml ='<TR><TD width="15%">DataField Name:</TD><TD colspan=3><INPUT TYPE="TEXT" NAME="'.$name.'_FieldData" VALUE="" SIZE="40" ONCHANGE="'.$name.'_FieldDataStore();">';
				$cmDataFieldhtml .='<BR>This is only for IM developer. Please dont change it. </TD></TR>';
				$cmDataFieldLoadJS ='theForm.'.$name.'_FieldData.value = '.$name.'_FieldSet['.$name.'_CurrentSelectedField].uuid;';
				
				$cmDataFieldStoreFunctionJS = "function {$name}_FieldDataStore() {theForm = document.forms.{$form};var fieldData = theForm.{$name}_FieldData.value;				
if ({$name}_CurrentSelectedIndex >= 0) {			
			{$name}_FieldSet[{$name}_CurrentSelectedField].uuid = fieldData;		
		}
}";
				$cmDataSelectFieldhtml = '<BR>UUID :<INPUT TYPE="TEXT" NAME="'.$name.'_OptionFieldData" VALUE="" SIZE="20" ONCHANGE=" '.$name.'_OptionFieldDataStore(true);" ><BR>This is only for IM developer';
				$cmDataOptionFieldLoadJS ='theForm.'.$name.'_OptionFieldData.value = '.$name.'_FieldSet['.$name.'_CurrentSelectedField].options['.$name.'_CurrentSelectedOptionIndex].uuid';								
				$cmDataOptionFieldStoreFunctionJS = "
	function {$name}_OptionFieldDataStore(storeit) {
		theForm = document.forms.{$form};		
		var value = theForm.{$name}_OptionFieldData.value;	
		alert({$name}_FieldSet[{$name}_CurrentSelectedField].options[{$name}_CurrentSelectedOptionIndex].uuid);		
		{$name}_FieldSet[{$name}_CurrentSelectedField].options[{$name}_CurrentSelectedOptionIndex].uuid = value;								
	}								";
			}
...
<!--
	...
	{$name}_CurrentSelectedOptionIndex = -1;
	...
	function {$name}_HideAndDisplayOptionSet() {
		...
		if (selectedFieldType == "MultiSelectFromArrayField" || 			
			selectedFieldType == "MultiCheckFromArrayField" ||
			selectedFieldType == "SelectFromArrayField" ||
			selectedFieldType == "ProductOptionsField" ||
			selectedFieldType == "RadioFromArrayField" ||
			selectedFieldType == "MonthlyScheduleField" ||
            selectedFieldType == "RadioWithOtherFromArrayField") {
	...
	function {$name}_Delete() {
		if (confirm("Deleting this field will erase this information from the database. Click OK to delete this field.") == true) {
	...
	function {$name}_OptionNew() {
		...
		fields[fields.length] = new Option('Untitled',uuid);	
		newindex = {$name}_FieldSet[{$name}_CurrentSelectedIndex].options.length;
		fields.selectedIndex = newindex;
		{$name}_FieldSet[{$name}_CurrentSelectedIndex].options[newindex] = new {$name}_Option('Untitled',uuid);		
	...
	function {$name}_OptionMove(how) {
		...
		if (((how == -1) && (selectedItem > 0))
			...
			// Swap the display first
			oldName = {$name}_FieldSet[{$name}_CurrentSelectedIndex].options[selectedItem].name;
			newName = {$name}_FieldSet[{$name}_CurrentSelectedIndex].options[selectedItem+how].name;

			{$name}_FieldSet[{$name}_CurrentSelectedIndex].options[selectedItem].name = newName;
			{$name}_FieldSet[{$name}_CurrentSelectedIndex].options[selectedItem+how].name = oldName;
			
			oldUUID = {$name}_FieldSet[{$name}_CurrentSelectedIndex].options[selectedItem].uuid;
			newUUID = {$name}_FieldSet[{$name}_CurrentSelectedIndex].options[selectedItem+how].uuid;
			
			{$name}_FieldSet[{$name}_CurrentSelectedIndex].options[selectedItem].uuid = newUUID;
			{$name}_FieldSet[{$name}_CurrentSelectedIndex].options[selectedItem+how].uuid = oldUUID;		
		}	
		{$name}_CurrentSelectedOptionIndex = fields.selectedIndex;			
	}
	...
	function {$name}_FieldLoad() {
		...
		theForm.{$name}_FieldComments.value = {$name}_FieldSet[{$name}_CurrentSelectedField].comments;
	...
	function {$name}_OptionFieldLoad() {
		...
		// Make sure everything is up to date before we wipe out those fields	
		//{$name}_OptionFieldValueStore(false);		
					
		theForm = document.forms.{$form};		
		
		var fields = theForm.{$name}_OptionFields;
		var selectedItem = fields.selectedIndex;
		{$name}_CurrentSelectedOptionIndex = selectedItem;
	...
	function {$name}_OptionFieldValueStore(storeit) {
		theForm = document.forms.{$form};		
		var value = theForm.{$name}_OptionFieldValue.value;
		//alert(value + " "+ theForm.{$name}_OptionFields.selectedIndex);
		
		if ({$name}_CurrentSelectedOptionIndex >= 0) {								
				{$name}_FieldSet[{$name}_CurrentSelectedField].options[{$name}_CurrentSelectedOptionIndex].name = value;
				theForm.{$name}_OptionFields.options[theForm.{$name}_OptionFields.selectedIndex].text = value;					
		}				
	
				
	}
	
	function {$name}_FieldCommentsStore() {
		theForm = document.forms.{$form};
		var value = theForm.{$name}_FieldComments.value;
		if ({$name}_CurrentSelectedIndex >= 0) {
			{$name}_FieldSet[{$name}_CurrentSelectedField].comments = value;
		}
	}
			...
			<TABLE BORDER="0" WIDTH="100%">
				...
				<TD VALIGN="TOP" align="Left" WIDTH="99%">
					...
					<TABLE BORDER="0" WIDTH="100%">
						...
						<TR>
							<TD width="15%">Name:</TD>
							<TD width="35%"><INPUT TYPE="TEXT" NAME="{$name}_FieldName" VALUE="" SIZE="40" ONCHANGE="{$name}_FieldNameStore();"></TD>
							<TD width="15%">Size/Columns:</TD>
							<TD width="35%"><INPUT TYPE="TEXT" NAME="{$name}_FieldSize" VALUE="" SIZE="20" ONCHANGE="{$name}_FieldSizeStore();"></TD>
						</TR>
						<TR>
							<TD>Type:</TD>
							<TD>
...
EOD;
						...
						if ($this->showComments) {
							$output .= "<TR><td valign=\"Top\">Comments:</td><TD COLSPAN=\"3\"><textarea name=\"{$name}_FieldComments\" cols=\"40\" rows=\"4\"  ONCHANGE=\"{$name}_FieldCommentsStore();\"></textarea></td></TR>";
						} else {
							$output .= "<input type='hidden' name=\"{$name}_FieldComments\" value=''>";
						}
							...
							<TD COLSPAN="3">													
								<TABLE width="400" cellpadding="1" cellspacing="1" border="0" id="{$name}_OptionFieldSet" style="display:none">	
									<TR>
										...
										<TD VALIGN="TOP">
											Value :
											<INPUT TYPE="TEXT" NAME="{$name}_OptionFieldValue" VALUE="" SIZE="30" ONCHANGE="{$name}_OptionFieldValueStore(true);" >
											$cmDataSelectFieldhtml
										</TD>
									</TR>
								</TABLE>
