...
* @author Dan Allen
*/
	function __findHtmlImages($images_dir)
	{
	...
	/* Revised _findHtmlImages() 
	 * Uses the image class by James Sleeman
	 * $images_dir now refers to a temporary directory
	 * used to store images while being attached.  Now 
	 * should be able to embed images from any URL
	 */
	...
	function _findHtmlImages($images_dir)
	{
		...
		foreach ($images[1] as $imageSrc) {
			if ( strcasecmp(substr($imageSrc, 0, 4), "http") != 0) {
				// Make the URL absolute, fine out first if the URL is 
				// absolute within the domain
				//if (substr($imageSrc, 0, 1) == '/') {
					// It is, just drop the domain infront 
					$absURL = $GLOBALS['cfg']['currentServer'] . $imageSrc;  
				//} else {
					// It's not, we'll have to prepend the domain and current directory
					//$absURL = "http" . ($_SERVER['SERVER_PORT'] == 447 ? 's' : '') . "://{$_SERVER['SERVER_NAME']}:{$_SERVER['SERVER_PORT']}" . dirname($PHP_SELF) . "/" . $imgSrc;
				//}			
			} else {
				$absURL = $imageSrc;
			}
			...
			// Open the image 
			$imageImg = new image($absURL);
			
			// Make a local file out of it
			$imageImg->makeTemporary($images_dir);
			
			// Add new name into the html_images and replace the old URL with the new name
			$html_images[] = basename($imageImg->src);
			$this->html = str_replace($imageSrc, basename($imageImg->src), $this->html);
		...
		if (!empty($html_images)) {
			...
			foreach ($html_images as $imageSrc) {
				$imageImg = new image("$images_dir/$imageSrc");
				$this->addHtmlImage($this->getFile("$images_dir/$imageSrc"), basename($imageImg->src), $imageImg->mimeType);
			}
		}
	}
