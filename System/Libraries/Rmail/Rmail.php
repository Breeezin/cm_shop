...
class Rmail
{
    ...
    */
    ...
    {
	return;	// test system, sendie emails
        if (!defined('RMAIL_CRLF')) {
        ...
        switch ($type) {
            ...
            case 'smtp':
				...
				$foo = $this->smtp_params;
                $smtp = &smtp::smtp_connect($foo);

				$foo =  $this->smtp_params['helo'];
                
                // Parse recipients argument for internet addresses
                foreach ($recipients as $recipient) {
					$foo =  $this->smtp_params['helo'];
                    $addresses = Mail_RFC822::newAddressList($recipient, $foo, null, false);
                ...
                foreach ($this->headers as $name => $value) {
                    if ($name == 'Cc' OR $name == 'Bcc') {
                        $addresses = Mail_RFC822::newAddressList($value, $foo, null, false);
                        foreach ($addresses as $address) {
                            $smtp_recipients[] = sprintf('%s@%s', $address->mailbox, $address->host);
                        }
                    }
                ...
                if (isset($this->return_path)) {
					...
					$foo = $this->headers['From'];
                    $from = Mail_RFC822::newAddressList($foo);
